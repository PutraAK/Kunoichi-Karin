...
(function () {
  ...
  if (!fs.existsSync(filePath)) {
    var defData = JSON.stringify(
      JsonEx.makeDeepCopy(Game_Particle.DEFAULT_DATA)
    );
...
(function () {
  ...
  (function () {
    ParticleEditor.GUIDE_TEXTS = {
      alpha: [
        ...
        null,
        LC("<中間値設定可能>", 2),
        LC("左端が開始時(時間0)、右端が終了時(時間1)の値", 3),
        LC("@時間(0~1) + Enterで中間値を設定可能", 4),
        LC("delete(Macはfn+delete)で中間値の削除", 5),
        LC("中間値編集中にshift+@時間(0~1)+Enterで時間変更", 6),
        LC("例)@0.5 + Enterで中間地点での値を設定開始", 7),
      ],
      scale: [
        LC("【スケール/拡大率】", 8),
        LC("1で等倍", 9),
        null,
        LC("<中間値設定可能>", 10),
        LC("左端が開始時(時間0)、右端が終了時(時間1)の値", 11),
        LC("@時間(0~1) + Enterで中間値を設定可能", 12),
        LC("delete(Macはfn+delete)で中間値の削除", 13),
        LC("中間値編集中にshift+@時間(0~1)+Enterで時間変更", 14),
        LC("例)@0.5 + Enterで中間地点での値を設定開始", 15),
      ],
      minimumScaleMultiplier: [
        LC("【最小スケール倍率】", 16),
        LC("パーティクル生成時の最小スケール倍率", 17),
        LC("例)0.1として開始時のスケールが0.5の場合は", 18),
        LC("　 0.5*0.1=0.05から0.5の間の", 19),
        LC("　 ランダムなスケールのパーティクルが発生。", 20),
      ],
      speed: [
        LC("【スピード/速度】", 21),
        LC("パーティクルの速度。開始値は0にしないこと。", 22),
        null,
        LC("<中間値設定可能>", 23),
        LC("左端が開始時(時間0)、右端が終了時(時間1)の値", 24),
        LC("@時間(0~1) + Enterで中間値を設定可能", 25),
        LC("delete(Macはfn+delete)で中間値の削除", 26),
        LC("中間値編集中にshift+@時間(0~1)+Enterで時間変更", 27),
        LC("例)@0.5 + Enterで中間地点での値を設定開始", 28),
      ],
      minimumSpeedMultiplier: [
        LC("【最小スピード倍率】", 29),
        LC("パーティクル生成時の最小スピード倍率", 30),
        LC("例)0.1として開始時のスピードが0.5の場合は", 31),
        LC("　 0.5*0.1=0.05から0.5の間の", 32),
        LC("　 ランダムなスピードのパーティクルが発生。", 33),
      ],
      color: [
        LC("【カラー/色】", 34),
        LC("左上のカラーピッカーで設定可能", 35),
        LC("バックスペース(Macはdelete)で白にリセット", 36),
        null,
        LC("<中間値設定可能>", 37),
        LC("左端が開始時(時間0)、右端が終了時(時間1)の値", 38),
        LC("@時間(0~1) + Enterで中間値を設定可能", 39),
        LC("delete(Macはfn+delete)で中間値の削除", 40),
        LC("中間値編集中にshift+@時間(0~1)+Enterで時間変更", 41),
        LC("例)@0.5 + Enterで中間地点での値を設定開始", 42),
      ],
      colorMode: [
        LC("【カラーモード】", 43),
        LC("0で時間によって色が変化。", 44),
        LC("0以外で発生時の色の分布を変化(時間変化無し)", 45),
      ],
      acceleration: [
        LC("【加速度】", 46),
        LC("加速度が有効な場合は画像の向きは移動方向で固定", 47),
      ],
      startRotation: [
        LC("【発生時の射出角度】", 48),
        LC("パーティクルの飛び出す方向。(画像の向きも連動)", 49),
        LC("0で右,90で下,180で左,270(-90)で上方向。", 50),
      ],
      rotationSpeed: [
        LC("【画像回転速度】", 51),
        LC("画像の回転速度の最小値と最大値。", 52),
        LC("動きには影響なし。", 53),
      ],
      imageOption: [
        LC("【画像表示設定】", 54),
        LC("画像の向き/反転の設定。動きには影響なし。", 55),
        LC("angle:0~360で画像のみ回転、-1でランダムな角度。", 56),
        LC("mirror:0で調整なし,1で反転,2でランダム。", 57),
      ],
      lifetime: [
        LC("【パーティクル寿命】", 58),
        LC("個々のパーティクルの消えるまでの秒数", 59),
      ],
      blendMode: [],
      frequency: [
        LC("【フリクエンシー/発生間隔】", 60),
        LC("パーティクルが発生する間隔", 61),
        LC("0.01とすると0.01秒に1回発生し、1秒間に100回", 62),
      ],
      spawnChance: [
        LC("【発生確率】", 63),
        LC("各発生タイミングに、パーティクルが発生する確率。", 64),
      ],
      particlesPerWave: [
        LC("【1回での発生個数】", 65),
        LC("各発生タイミングに同時に発生する個数。", 66),
        LC("同時に発生するパーティクルのlifetimeは固定", 67),
      ],
      emitterLifetime: [
        LC("【エミッター寿命】", 68),
        LC("-1で無限。0より大きくすると、指定時間で発生終了", 69),
      ],
      maxParticles: [
        LC("【最大表示数の制限】", 70),
        LC("このエミッターから発生する最大数の制限値", 71),
      ],
      fluctuation: [
        LC("【フラクチュエーション/ゆらぎ】", 72),
        LC("角度のゆらぎ設定。", 73),
        LC("max:各フレームで角度の変化幅の最大値", 74),
        LC(
      ...
      pos: [
        LC("【ポイント(発生方法)】", 76),
        LC("指定の点から発生。", 77),
        LC("<※発生方法はいずれか１つのみ指定が有効>", 78),
      ],
      rect: [
        LC("【レクト/四角(発生方法)】", 79),
        LC("四角形の中からランダムに発生。", 80),
        LC("左上の点をx,y、幅/高さをwidth,heightで指定", 81),
        LC("<※発生方法はいずれか１つのみ指定が有効>", 82),
      ],
      circle: [
        LC("【サークル/円(発生方法)】", 83),
        LC("円の中からからランダムに発生", 84),
        LC("中心点をx,y、半径をradiusで指定。", 85),
        LC("<※発生方法はいずれか１つのみ指定が有効>", 86),
      ],
      ring: [
        LC("【リング(発生方法)】", 87),
        LC("ドーナツ状のリングからランダムに発生。", 88),
        LC("中心点をx,y、外側の半径をradius", 89),
        LC("内側の半径をminRで指定。", 90),
        LC("<※発生方法はいずれか１つのみ指定が有効>", 91),
      ],
      burst: [
        LC("【バースト(発生方法)】", 92),
        LC("particlesPerWaveで規則的に発生させる", 93),
        LC("ps:射出角度の差(particleSpacing)", 94),
        LC("sa:開始角度。ps*sa=360とすると放射状に射出(startAngle)", 95),
        LC("dt:射出時間の差(deltaTime)", 96),
        LC("rt:0=開始角度(startRotation)無視", 97),
        LC("└1=開始角度優先,2=開始角度を加算(rotationType)", 98),
        LC("r:半径(radius)", 99),
        LC("dr:半径の差。マイナスで1周ごとの差(deltaRadius)", 100),
        LC("dx:xの差(deltaX)", 101),
        LC("dy:yの差(deltaY)", 102),
        LC("<※発生方法はいずれか１つのみ指定が有効>", 103),
      ],

      sub: [
        LC("【サブエミッター設定】", 104),
        LC("st:射出開始タイミング(0~1)", 105),
        LC("et:射出終了タイミング(0~1。-1で無効)", 106),
        LC("spd:慣性の影響度合い。0=影響なし、1=100%影響。", 107),
        LC("iAngle:1=開始角度は慣性方向基準", 108),
        LC("iScale:1=親パーティクルの拡大率乗数を継承", 109),
      ],
    };
  ...
  Game_Particle.prototype._particleEdit = function (
    ...
    if (!data) {
      if (!subId) {
        ...
        throw new Error(
          LC("管理ID「%1」は再生されてません。", 111).format(mainId)
        );
      }
    }
  ...
  Scene_Boot.prototype.start = function () {
    ...
    PresetPicker.CATEGORY_NAMES = [
      NLC("キャラ対象", 158, "character"),
      NLC("スクリーン/天候/リージョン", 159, "screen/weather/region"),
      NLC("マウス/タップ", 160, "click/drag"),
      NLC("その他", 161, "Others"),
      NLC("非表示(/h)", 162, "hidden(/h)"),
    ];
  };
...
===================================*/
  ...
  ParticleEditor.prototype.writeHelpFile = function () {
    ...
    file +=
      " * @" + "plugindesc " + LC("自作パーティクル設定の一覧", 122) + "\n";
    file += " * @" + "help\n";
...
===================================*/
  ...
  PickerBase.prototype.onKeyDown = function (keyCode, event) {
    if (
      !event.ctrlKey &&
      !event.metaKey &&
      keyCode >= KEY_CODE.alphabet &&
      keyCode <= KEY_CODE.alphabetEnd
    ) {
      var chara = event.key;
      this.search(chara);
    }
...
===================================*/
  ...
  PresetPicker.prototype.itemWidth = function () {
    return $gameSystem.isJapanese()
      ? PresetPicker.COL_WIDTH
      : PresetPicker.COL_WIDTH_ENG;
  };
