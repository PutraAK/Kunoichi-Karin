...
Moghunter.enemyhp_duration = Number(
  Moghunter.parameters["EHP Fade Duration"] || 120
);
...
// * CreateLowerLayer
//==============================
var _alias_mog_enemyhp_createUpperLayer =
...
Spriteset_Battle.prototype.create_ehp_sprites = function () {
  ...
  for (var i = 0; i < this._enemySprites.length; i++) {
    if (this._enemySprites[i]._battler == null) {
      return;
    }
    ...
    if (!this._enemySprites[i]._battler._hp_meter) {
      this._ehp_sprites_a[i].visible = false;
    }
    ...
    this._ehp_sprites_data[i] = [
      this._enemySprites[i]._battler.hp,
      0,
      0,
      Math.max(Moghunter.enemyhp_duration, 32),
      this._enemySprites[i]._battler.hp,
    ];
  }
};
...
Spriteset_Battle.prototype.update_ehp_sprites = function () {
  for (var i = 0; i < this._enemySprites.length; i++) {
    if (this._ehp_sprites_a[i] == null) {
      return;
    }
    if (!this._ehp_sprites_a[i].bitmap.isReady()) {
      return;
    }
    this._ehp_sprites_a[i].x =
      this._enemySprites[i].x +
      Moghunter.enemyhp_a_x +
      this._ehp_sprites_data[i][1] -
      this._ehp_sprites_a[i].bitmap.width / 2;
...
Spriteset_Battle.prototype.update_ehp_blue_bar = function (i) {
  ...
  var meter_rate =
    (this._ehp_sprites_c[i].bitmap.width * this._enemySprites[i]._battler.hp) /
    this._enemySprites[i]._battler.mhp;
  this._ehp_sprites_c[i].setFrame(
    0,
    0,
    meter_rate,
    this._ehp_sprites_c[i].bitmap.height / 2
  );
};
...
Spriteset_Battle.prototype.update_ehp_red_bar = function (i) {
  if (this._ehp_sprites_data[i][4] != this._enemySprites[i]._battler.hp) {
    ...
    var meter_rate =
      (this._ehp_sprites_b[i].bitmap.width * this._ehp_sprites_data[i][4]) /
      this._enemySprites[i]._battler.mhp;
    this._ehp_sprites_b[i].setFrame(
      0,
      this._ehp_sprites_b[i].bitmap.height / 2,
      meter_rate,
      this._ehp_sprites_b[i].bitmap.height / 2
    );
  }
