...
(function () {
  ...
  var user_fix_encount = Boolean(
    parameters["Encount Effect"] === "true" || false
  );
  var user_use_camera = Boolean(
    parameters["Camera Controll"] === "true" || false
  );
  var user_use_camera_transfer = Boolean(
    parameters["Camera Controll"] === "minimum" || false
  );
  var user_fix_weather = Boolean(
    parameters["Weather Patch"] === "true" || false
  );
  ...
  var dp_renderSize = {
    ...
    onChange: function (_scale) {
      ...
      spriteset._tilemap.width =
        Math.ceil(Graphics.width / scale) + spriteset._tilemap._margin * 2;
      spriteset._tilemap.height =
        Math.ceil(Graphics.height / scale) + spriteset._tilemap._margin * 2;

      //パララックスサイズ変更
      spriteset._parallax.move(
        0,
        0,
        Math.round(Graphics.width / scale),
        Math.round(Graphics.height / scale)
      );

      // Foreground.js対応
      if (
        spriteset._foreground &&
        spriteset._foreground instanceof TilingSprite
      ) {
        spriteset._foreground.move(
          0,
          0,
          Math.round(Graphics.width / scale),
          Math.round(Graphics.height / scale)
   ...
   */
  var _p_dpfocus =
    "dpFocus" in drowsepost.fn ? drowsepost.fn.dpFocus : function () {};
   ...
   */
  var _p_dpzoom =
    "dpZoom" in drowsepost.fn ? drowsepost.fn.dpZoom : function () {};
  ...
  (function () {
    ...
    Game_Map.prototype.screenTileX = function () {
      return (
        (Graphics.width - user_map_marginright) /
        (this.tileWidth() * $gameScreen.zoomScale())
      );
    };
    ...
    Game_Map.prototype.screenTileY = function () {
      return (
        (Graphics.height - user_map_marginbottom) /
        (this.tileHeight() * $gameScreen.zoomScale())
      );
    };
  ...
  (function () {
    ...
    Game_Event.prototype.updateScroll = function (
      lastScrolledX,
      lastScrolledY
    ) {
      if (typeof $gameMap !== "object") return;
      if ($gameMap._dp_target !== this._eventId) return;
      Game_Character.prototype.updateScroll.call(
        this,
        lastScrolledX,
        lastScrolledY
      );
  ...
  (function () {
    ...
    Weather.prototype._rebornSprite = function (sprite) {
      ...
      sprite.ax =
        Math.randomInt(dp_renderSize.width + 100) - 50 + this.origin.x;
      sprite.ay =
        Math.randomInt(dp_renderSize.height + 200) - 100 + this.origin.y;
  ...
  (function () {
    ...
    Scene_Map.prototype.start = function () {
      ...
      if (this._transfer) {
        ...
        //イベントエディタからのテスト実行では$gameMap.metaが定義されない。
        $gameMap._dp_target =
          "meta" in $dataMap ? Number($dataMap.meta.camTarget || 0) : 0;
      ...
      //画面中心を強制設定する
      if (!user_use_camera && user_use_camera_transfer)
        camera.center(null, null, true);
    };
