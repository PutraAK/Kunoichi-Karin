...
(function () {
  ...
  const pShowTargetEnemyName = toBoolean(
    parameters["ShowTargetEnemyName"],
    true
  );
  const pEnemyNameWindowHeight = toNumber(
    parameters["EnemyNameWindowHeight"],
    72
  );
  const pEnemyNameWindowOpacity = toNumber(
    parameters["EnemyNameWindowOpacity"],
    255
  );
  const pEnemyNameWindowAdjustX = toNumber(
    parameters["EnemyNameWindowAdjustX"],
    0
  );
  const pEnemyNameWindowAdjustY = toNumber(
    parameters["EnemyNameWindowAdjustY"],
    0
  );
  ...
  const pSwitchActorLeftRight = toBoolean(
    parameters["SwitchActorLeftRight"],
    true
  );
  ...
  const pActorCursorPosition = setDefault(
    parameters["ActorCursorPosition"],
    pEnemyCursorPosition
  );
  const pActorCursorAdjustX = parameters["ActorCursorAdjustX"];
  const pActorCursorAdjustY = toNumber(
    parameters["ActorCursorAdjustY"],
    pEnemyCursorAdjustY
  );
  const pShowTargetActorName = toBoolean(
    parameters["ShowTargetActorName"],
    pShowTargetEnemyName
  );
  const pActorNameWindowHeight = toNumber(
    parameters["ActorNameWindowHeight"],
    pEnemyNameWindowHeight
  );
  const pActorNameWindowOpacity = toNumber(
    parameters["ActorNameWindowOpacity"],
    pEnemyNameWindowOpacity
  );
  const pActorNameWindowAdjustX = parameters["ActorNameWindowAdjustX"];
  const pActorNameWindowAdjustY = toNumber(
    parameters["ActorNameWindowAdjustY"],
    pEnemyNameWindowAdjustY
  );
  const pActorNameFontSize = toNumber(
    parameters["ActorNameFontSize"],
    pEnemyNameFontSize
  );
  const pActorNameTextAdjustY = toNumber(
    parameters["ActorNameTextAdjustY"],
    pEnemyNameTextAdjustY
  );
  ...
  Window_BattleEnemyName.prototype.select = function (index) {
    ...
    if (enemy) {
      ...
      const textWidth = this.textWidth(
        this.convertEscapeCharacters(enemy.name())
      );
      // ウィンドウの余白と文字の余白を加算
      let textPaddingWidth =
        textWidth + this.padding * 2 + itemPadding(this) * 2;
      // 根拠不明だが調整
      // ※加算しないと文字が小さい……。
      textPaddingWidth += 16;
  ...
  Window_BattleActorName.prototype.select = function (index) {
    ...
    if (actor) {
      ...
      // ウィンドウ幅を求める。
      const textWidth = this.textWidth(
        this.convertEscapeCharacters(actor.name())
      );
      // ウィンドウの余白と文字の余白を加算
      let textPaddingWidth =
        textWidth + this.padding * 2 + itemPadding(this) * 2;
  ...
  Spriteset_Battle.prototype.createLowerLayer = function () {
    ...
    if (pEnemyTargetCursor) {
      ...
      this._enemyCursorSprite.bitmap =
        ImageManager.loadPicture(pEnemyTargetCursor);
      ...
      if (!pActorTargetCursor) {
        ...
        this._actorCursorSprite.bitmap =
          ImageManager.loadPicture(pEnemyTargetCursor);
        // 画像を左右反転（scaleX = -1）する。
    ...
    if (pActorTargetCursor) {
      this._actorCursorSprite = new Sprite();
      this._actorCursorSprite.bitmap =
        ImageManager.loadPicture(pActorTargetCursor);
      this._baseSprite.addChild(this._actorCursorSprite);
      this._actorCursorSprite.hide();
    }
  };
