...
(function () {
  ...
  Scene_Debug.prototype.update = function () {
    ...
    if (Input.isTriggered("S")) {
      ...
      if (this.strNum && this.strNum.length > 0)
        this.strNum = this.strNum.substr(0, this.strNum.length - 1);
      this.jump();
    }
    for (var i = 0; i <= 9; i++) {
  ...
  Scene_Debug.prototype.jump = function () {
    ...
    var rangeIndex =
      this.shortCutType == "S" ? 0 : this._rangeWindow._maxSwitches;
    if (this.strNum != "") {
      var num = parseInt(this.strNum);
      if (
        (this.shortCutType == "S" && num > $dataSystem.switches.length - 1) ||
  ...
  Scene_Debug.prototype.refreshHelpWindow = function () {
    ...
    this._debugHelpWindow.drawTextEx(
      str,
      450,
      this._debugHelpWindow.lineHeight() * 4
    );
  };
})();
