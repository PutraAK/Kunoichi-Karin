...
(function () {
  ...
  DataManager.cutIn = function (item) {
    ...
    for (var i = 0, max = texts.length; i < max; i++) {
      if (/<(?:カットイン|CutIn):(.+)>/.exec(texts[i])) {
        ary.push(RegExp.$1.split(" "));
      }
    }
    return ary;
  };
  ...
  Window_BattleLog.prototype.showWindows = function () {
    ...
    if (Imported["TurnWindow"]) {
      SceneManager._scene._turnWindow.show();
    }
    if (Imported["yAutoBattle"]) {
      SceneManager._scene._autoBattleWindow.show();
    }
  };
  ...
  CommonPopupManager.hideWindows = function () {
    if ($gameParty.inBattle()) {
      SceneManager._scene._logWindow._showStatus =
        SceneManager._scene._logWindow.visible;
      SceneManager._scene._logWindow.hide();
      SceneManager._scene._statusWindow.hide();
      if (Imported["TurnWindow"]) {
        SceneManager._scene._turnWindow.hide();
      }
      if (Imported["yAutoBattle"]) {
        SceneManager._scene._autoBattleWindow.hide();
      }
    }
  };
