...
(() => {
  ...
  Game_Enemy.prototype.makeActions = function () {
    ...
    for (let i = 0; i < this.numActions(); i++) {
      ...
      if (this._multiActionList[i]) {
        const actionList = this._multiActionList[i].filter((a) =>
  ...
  Game_Enemy.prototype.makeMultiActionList = function () {
    ...
    this._lastActionList = this.parseActionList(
      this.enemy().meta.LastAction
    ).map((n) => this.enemy().actions[n - 1]);
    // get meta data and initialize multi action list
    this._multiActionList = [];
    const multiActions = [];
    for (let i = 1; i <= MaxMultiAction; i++) {
      multiActions.push(
        this.parseActionList(this.enemy().meta["MultiAction" + i])
      );
