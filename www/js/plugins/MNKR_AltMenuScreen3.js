...
(function () {
  ...
  Window_MenuStatus.prototype.maxCols = function () {
    var realMaxColsMenuWnd =
      maxColsMenuWnd === 0 ? $gameParty.members().length : maxColsMenuWnd;
  ...
  Window_MenuStatus.prototype.drawItem = function (index) {
    ...
    const charaIndex = $gameActors.actor(1)._characterIndex;
    const charaName = $gameActors.actor(1)._characterName;

    // ■■■ Altered Mod ■■■
    if (bitmapName === $dataActors[1].meta.stand_picture) {
      if (
        $gameSwitches.value(686) ||
        (charaIndex === 6 && charaName === "bu_001")
      ) {
        bitmapName = $dataActors[actor.actorId()].meta.stand_picture2;
      }
      if (
        $gameSwitches.value(3) ||
        (charaIndex === 1 && charaName === "bu_001")
      ) {
        bitmapName = $dataActors[actor.actorId()].meta.stand_picture3;
      }
    }
  ...
  Window_MenuStatus.prototype.drawItemImage = function (index) {
    ...
    var bitmap = bitmapName ? ImageManager.loadPicture(bitmapName) : null;
    var offX = $dataActors[actor.actorId()].meta.stand_offset_x
      ? Number($dataActors[actor.actorId()].meta.stand_offset_x)
      : 0;
    var offY = $dataActors[actor.actorId()].meta.stand_offset_y
      ? Number($dataActors[actor.actorId()].meta.stand_offset_y)
      : 0;
  ...
  Window_MenuStatus.prototype.drawItemStatus = function (index) {
    ...
    if (SceneManager._scene.constructor.name === "Scene_Item") {
      this.drawActorPrams(actor, x, bottom - lineHeight * 7, width, 2);
      this.drawActorPrams(actor, x, bottom - lineHeight * 6, width, 4);
      this.drawActorPrams(actor, x, bottom - lineHeight * 5, width, 6);
      this.drawActorPrams(actor, x, bottom - lineHeight * 7, width, 3);
      this.drawActorPrams(actor, x, bottom - lineHeight * 6, width, 5);
      this.drawActorPrams(actor, x, bottom - lineHeight * 5, width, 7);
    }
  ...
  // ■■■ Altered Mod ■■■
  Window_MenuStatus.prototype.drawActorPrams = function (
    actor,
    _x,
    y,
    width,
    ...
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.param(paramId), x + x2, y, 72);
    this.resetTextColor();
    this.drawText(actor.param(paramId), x + x2, y, width, "right");
  };
  ...
  Bitmap.prototype.bltStand = function (
    source,
    sx,
    sy,
    sw,
