...
(function () {
  ...
  var enabledStatusWhenSelectActor = paramParse(
    parameters["Hide When Selecting Actor"]
  );
  var enabledStatusWhenSelectEnemy = paramParse(
    parameters["Hide When Selecting Enemy"]
  );
  ...
  if (enabledStatusWhenSelectActor) {
    var _BWL_Scene_Battle_selectActorSelection =
  ...
  if (enabledStatusWhenSelectEnemy) {
    var _BWL_Scene_Battle_selectEnemySelection =
  ...
  Window_PartyCommand.prototype.initialize = function () {
    var x =
      (+FTKR.BWL.partyCmdPosX * (Graphics.boxWidth - this.windowWidth())) / 2;
    ...
    switch (+FTKR.BWL.partyCmdPosY) {
      ...
      default:
        var y =
          Graphics.boxHeight -
          this.statusWindow().windowHeight() -
          this.windowHeight();
        break;
    }
    this.y = y;
    ...
    switch (FTKR.BWL.actorCmdPos) {
      ...
      default: //重ね
        var y =
          Graphics.boxHeight -
          this.statusWindow().windowHeight() +
          (FTKR.BWL.actorCmdPosY *
            (this.statusWindow().windowHeight() - this.windowHeight())) /
            2;
        break;
    }
    var x = this.offsetX();
  ...
  Window_ActorCommand.prototype.offsetX = function () {
    return (
      (+FTKR.BWL.actorCmdPosX *
        ((FTKR.BWL.actorCmdPos == 1 ? Graphics.boxWidth : this.actorWidth()) -
          this.windowWidth())) /
      2
    );
  };
  ...
  Window_ActorCommand.prototype.actorWidth = function () {
    return (
      SceneManager._scene._statusWindow.width /
      SceneManager._scene._statusWindow.maxCols()
    );
  };
  ...
  Window_ActorCommand.prototype.refreshPosition = function () {
    if (FTKR.BWL.actorCmdPos != 1)
      this.x =
        this.actorWidth() * BattleManager.actor().index() + this.offsetX();
  };

  Window_ActorCommand.prototype.numVisibleRows = function () {
    return (
      +FTKR.BWL.actorCmdWindow.height || (FTKR.BWL.actorCmdPos == 1 ? 1 : 4)
    );
  };
