...
(function (global) {
  ...
  const highlightSettings = JSON.parse(pluginParameters["Highlight Words"]).map(
    (e) => JSON.parse(e)
  );
  const autoHighlightWindows = JsonEx.parse(
    pluginParameters["Auto highlight Windows"] || '["Window_Message"]'
  ).map((parsed) => String(parsed));
  const skillHighlight = {
    color: String(pluginParameters["Skill Name Color"] || 0),
    skillIds: JsonEx.parse(pluginParameters["Highlight Skills"] || "[]").map(
  ...
  class HighlightWords {
    ...
    refreshCache() {
       ...
       */
      this._sortedWords = this._highlightWords
        .map((word) => word.word)
    ...
    findColorByWord(word) {
      ...
      return this._colors[word].startsWith("#")
        ? this._colors[word]
        : Number(this._colors[word]);
    }
  ...
  DataManager.onLoad = function (object) {
    ...
    if (object === $dataSkills) {
      skillHighlight.skillIds
        .map(
          (skillId) =>
            new HighlightWord($dataSkills[skillId].name, skillHighlight.color)
        )
  ...
  Window_Base.prototype.isHighlightWindow = function () {
    return autoHighlightWindows.some(
      (autoHighlightWindow) =>
        typeof window[autoHighlightWindow] === "function" &&
