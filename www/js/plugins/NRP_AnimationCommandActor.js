...
/*:
 * @target MZ
 * @plugindesc v1.002 Display an animation for the actor at the time of command input.
...
/*:ja
 * @target MZ
 * @plugindesc v1.002 コマンド入力時のアクターにアニメーションを表示する。
...
(function () {
  ...
  const pStartAnimationWait = Math.max(
    toNumber(parameters["startAnimationWait"], 1),
    1
  );
  const pStartBackground = toBoolean(
    parameters["startAnimationBackground"],
    false
  );
  ...
  const pLoopBackground = toBoolean(
    parameters["loopAnimationBackground"],
    false
  );
  ...
  function removeLoopAnimation() {
    ...
    if (Utils.RPGMAKER_NAME == "MV") {
      var battlerSprites = BattleManager._spriteset._actorSprites.concat(
      ...
      for (const battlerSprite of battlerSprites) {
        // 削除対象のアニメーションを一旦格納
        let removeList = battlerSprite._animationSprites.filter(function (
          spriteAnimation
        ) {
          // 再生中のループアニメ
   ...
   * MZ用のアニメーション呼び出し
   */
  function createAnimationSprite(
    targets,
    animation,
    mirror,
    delay,
