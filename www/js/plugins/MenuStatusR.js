...
(function () {
  ...
  // 再定義
  var __WBase_drawActorSimpleStatus =
  ...
  Window_Base.prototype.drawStatusContents = function (actor, rect) {
    ...
    for (var i = 0, max = pages[this._pageIndex].length; i < max; i++) {
      ...
      switch (code) {
        case "name":
          this.drawActorName(actor, xx, yy);
          break;
        case "lv":
          this.drawActorLevel(actor, xx, yy);
          break;
        case "states":
          this.drawActorIcons(actor, xx, yy, width);
          break;
        case "class":
          this.drawActorClass(actor, xx, yy, width);
          break;
        case "nickname":
          this.drawActorNickname(actor, xx, yy, width);
          break;
        case "hp":
          this.drawActorHp(actor, xx, yy, width);
          break;
        case "mp":
          this.drawActorMp(actor, xx, yy, width);
          break;
        case "tp":
          this.drawActorTp(actor, xx, yy, width);
          break;
        case "exp":
          this.drawLineActorExp(actor, xx, yy, width);
          break;
        case "next":
          this.drawLineActorNextExp(actor, xx, yy, width);
          break;
        default:
          this.drawLineParameter(actor, code, xx, yy, width);
      }
    }
  };

  Window_Base.prototype.drawLineParameter = function (
    actor,
    code,
    xx,
    yy,
      ...
      if (code.match(/^q(\d+)/i)) {
        ...
        if (equip) {
          this.drawIcon(
            equip.iconIndex,
            xx,
            yy + (this.lineHeight() - this.standardFontSize()) / 2 - 2
          );
        ...
        this.drawText(
          Math.floor(actor.elementRate(id) * 100) + "%",
          xx,
          yy,
          width,
          "right"
        ...
        this.changeTextColor(this.normalColor());
        this.drawText(
          Math.floor(actor.stateRate(id) * 100) + "%",
          xx,
          yy,
          width,
          "right"
        ...
        this.changeTextColor(this.normalColor());
        this.drawText(
          Math.floor(actor.debuffRate(id) * 100) + "%",
          xx,
          yy,
          width,
          "right"
  ...
  Window_Base.prototype.prevPage = function () {
    this.changePage(
      (this._pageIndex + (this.maxPages() - 1)) % this.maxPages()
    );
  };
  ...
  Window_Base.prototype.maxBattleMembers = function () {
    if (Imported["SceneFormation"]) {
      return Number(
        PluginManager.parameters("SceneFormation")["Max Battle Members Size"] ||
          4
      );
    } else {
  ...
  Window_MenuStatus.prototype.lineHeight = function () {
    ...
    h = Math.floor(
      Math.min(this.contentsHeight() / (this.maxBattleMembers() * maxRows), h)
    );
    return h - 2;
  };
  ...
  // 再定義
  Window_MenuStatus.prototype.processDrawIcon = function (
    iconIndex,
    textState
  ) {
  ...
  Scene_Menu.prototype.createCommandWindow = function () {
    ...
    this._commandWindow.setHandler(
      "right",
      this.changeStatus.bind(this, "right")
    );
    this._commandWindow.setHandler(
      "left",
      this.changeStatus.bind(this, "left")
    );
  };
  ...
  if (useSkillStatus) {
    ...
    // 再定義
    Window_SkillStatus.prototype.processDrawIcon = function (
      iconIndex,
      textState
    ) {
      this.drawIcon(iconIndex, textState.x + 2, textState.y + 2);
    ...
    var __SSkill_createSkillTypeWindow =
