...
(function () {
  ...
  Window_Base.prototype.drawIcon = function (iconIndex, x, y) {
    if (this.contents.fontSize === 27) {
      ...
      var n = Math.floor(
        (this.contents.fontSize / 28) * Window_Base._iconWidth
      );
      var nn = (32 - n) / 2;
      let adjSx = 0;
      if (sx === 0) adjSx = 1;
      this.contents.blt(bitmap, sx - 1 + adjSx, sy, pw, ph, x + 4, y, n, n);
    } else {
      var bitmap = ImageManager.loadSystem("IconSet");
      var pw = Window_Base._iconWidth;
      var ph = Window_Base._iconHeight;
  ...
  Window_ChoiceList.prototype.drawIcon = function (iconIndex, x, y) {
    var bitmap = ImageManager.loadSystem("IconSet");
    var pw = Window_Base._iconWidth;
    var ph = Window_Base._iconHeight;
    var sx = (iconIndex % 16) * pw;
  ...
  Window_Message.prototype.processDrawIcon = function (iconIndex, textState) {
    let y = textState.y;
    if (textState.y === 0) y += 3;
    this.drawIcon(iconIndex, textState.x + 2 + 3, y + 2);
    var n = Math.floor(
      (this.contents.fontSize / this.standardFontSize()) *
        Window_Base._iconWidth
    );
  ...
  Window_ChoiceList.prototype.processDrawIcon = function (
    iconIndex,
    textState
  ) {
    this.drawIcon(iconIndex, textState.x + 2 + 3, textState.y + 2 - 1);
    var n = Math.floor(
      (this.contents.fontSize / this.standardFontSize()) *
        Window_Base._iconWidth
    );
