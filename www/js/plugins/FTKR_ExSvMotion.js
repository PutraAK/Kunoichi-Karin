...
(function () {
  ...
  FTKR.ESM.skipPartyOn =
    paramParse(parameters["Enabled Refresh In PartyCmd"]) || false;
  FTKR.ESM.motion = {
    debug: {
  ...
  Game_Actor.prototype.performAction = function (action) {
    ...
    if (
      motion &&
      testObjectMeta(action.item(), ["ESM 武器モーション", "ESM WEAPON_MOTION"])
    ) {
  ...
  //書き換え
  var _ESM_Game_BattlerBase_stateMotionIndex =
    Game_BattlerBase.prototype.stateMotionIndex;
  ...
  Sprite_Battler.prototype.convertOtherMotion = function (other) {
    return other && other.match(/other(\d+)/)
      ? Sprite_Battler.ESM_MOTION_NAME[Number(RegExp.$1)]
      : other;
  };
  ...
  Sprite_Battler.prototype.lastMotionType = function () {
    return (
      this.motions().length === 1 || this.motions().length <= this._index + 1
    );
  };
  ...
  Sprite_Battler.prototype.esmUpdateMotionCount = function (battler) {
    ...
    if (this.motion() && ++this._motionCount >= this.motionSpeed()) {
      ...
      var loopFrames =
        Imported.YED_SideviewBattler && battler.isSideviewBattler()
          ? frames
          : frames + 1;
      // ループする場合
  ...
  if (Imported.YEP_BattleEngineCore) {
    ...
    var _ESM_Game_Battler_requestMotionRefresh =
    ...
    Sprite_Battler.prototype.checkBattleInputActorUndecided = function (
      battler
    ) {
