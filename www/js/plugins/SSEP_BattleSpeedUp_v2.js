...
BattleManager.initMembers = function () {
  ...
  this._battleSpeedReset = false;
};

const _BattleManager_processVictory = BattleManager.processVictory;
BattleManager.processVictory = function () {
  _BattleManager_processVictory.call(this);
  this._battleSpeedReset = true;
};

BattleManager.BattleSpeedDefault = function () {
  let speed = Sepher.Param.BattleSpeedDefault;
  if (BattleManager._battleSpeedReset) speed = 1;
  return speed;
};

BattleManager.BattleSpeedBoost = function () {
  let speed = Sepher.Param.BattleSpeedBoost;
  if (BattleManager._battleSpeedReset) speed = 1;
  return speed;
};
///////////////////////////////////////////////////////////////////////////////
...
Sprite_StateIcon.prototype.animationWait = function () {
  var speed = Sepher.Param.StateIcon / BattleManager.BattleSpeedDefault();
  return speed;
};

Sprite_StateOverlay.prototype.animationWait = function () {
  var speed = Sepher.Param.StateOverlay / BattleManager.BattleSpeedDefault();
  return speed;
};

Sprite_Weapon.prototype.animationWait = function () {
  var speed = Sepher.Param.Weapon / BattleManager.BattleSpeedDefault();
  if (Sepher.Param.OkayKeyBoost) {
    if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
      speed = speed / BattleManager.BattleSpeedBoost();
    } else {
      speed = speed / BattleManager.BattleSpeedDefault();
    }
  } else {
    if (QuickMode) {
      speed = speed / BattleManager.BattleSpeedBoost();
...
Sprite_Balloon.prototype.waitTime = function () {
  var speed = Sepher.Param.Balloon / BattleManager.BattleSpeedDefault();
  if (Sepher.Param.OkayKeyBoost) {
    if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
      speed = speed / BattleManager.BattleSpeedBoost();
    } else {
...
Sprite_Battler.prototype.startMove = function (x, y, duration) {
  if (Sepher.Param.OkayKeyBoost) {
    if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
      duration = duration / BattleManager.BattleSpeedBoost();
    } else {
      duration = duration / BattleManager.BattleSpeedDefault();
    }
  } else {
    if (QuickMode) {
      duration = duration / BattleManager.BattleSpeedBoost();
...
Sprite_Animation.prototype.setup = function (target, animation, mirror, delay) {
  if (SceneManager._scene.constructor.name === "Scene_Map") {
    ...
    delay = delay / BattleManager.BattleSpeedDefault();
    if (Sepher.Param.OkayKeyBoost) {
      if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
        delay = delay / BattleManager.BattleSpeedBoost();
      } else {
        delay = delay / BattleManager.BattleSpeedDefault();
      }
    } else {
...
Sprite_Animation.prototype.setupRate = function () {
  if (SceneManager._scene.constructor.name === "Scene_Map") {
    this._rate =
      $gameVariables.value(160) === "0" || $gameVariables.value(160) === 0
        ? 2
        : $gameVariables.value(160); // 160:アニメ速さ　数値　指定
  } else {
    _SSEP_Animation_setupRate;
    if (Sepher.Param.OkayKeyBoost) {
      if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
        this._rate = this._rate / BattleManager.BattleSpeedBoost();
      } else {
        this._rate = this._rate / BattleManager.BattleSpeedDefault();
      }
    } else {
      if (QuickMode) {
        this._rate = this._rate / BattleManager.BattleSpeedBoost();
...
Sprite_Damage.prototype.initialize = function () {
  ...
  if (Sepher.Param.OkayKeyBoost) {
    if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
      this._duration = this._duration / BattleManager.BattleSpeedBoost();
    } else {
      this._duration = this._duration / BattleManager.BattleSpeedDefault();
    }
  } else {
    if (QuickMode) {
      this._duration = this._duration / BattleManager.BattleSpeedBoost();
...
if (Imported.YEP_BattleEngineCore) {
  ...
  BattleManager.actionPerformAction = function () {
    ...
    if (Sepher.Param.OkayKeyBoost) {
      if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
        wait = wait / BattleManager.BattleSpeedBoost();
      } else {
        wait = wait / BattleManager.BattleSpeedDefault();
      }
    } else {
      if (QuickMode) {
        wait = wait / BattleManager.BattleSpeedBoost();
  ...
  BattleManager.actionFloat = function (name, actionArgs) {
    ...
    if (Sepher.Param.OkayKeyBoost) {
      if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
        frames = frames / BattleManager.BattleSpeedBoost();
      } else {
        frames = frames / BattleManager.BattleSpeedDefault();
      }
    } else {
      if (QuickMode) {
        frames = frames / BattleManager.BattleSpeedBoost();
    ...
    if (Sepher.Param.YEPVictoryWait) {
      if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
        return (
          ...
          Yanfly.Param.VACheerWait / BattleManager.BattleSpeedBoost()
        );
      } else {
        return (
          ++this._victoryCheerWait >=
          Yanfly.Param.VACheerWait / BattleManager.BattleSpeedDefault()
        );
      }
    } else {
