...
(function () {
  ...
  function readArgs(params, args) {
    ...
    if (args.Option) {
      ...
      params.dimmerLevel = setArg(
        toNumber(option.DimmerLevel),
        params.dimmerLevel
      );
      params.image = setArg(option.Image, params.image);
      params.color = setArg(option.Color, params.color);
      params.individualBitmap = setArg(
        toBoolean(option.IndividualBitmap),
        params.individualBitmap
      );
      ...
      params.adjustStartX = setArg(
        toNumber(option.AdjustStartX),
        params.adjustStartX
      );
      params.adjustStartY = setArg(
        toNumber(option.AdjustStartY),
        params.adjustStartY
      );
      ...
      params.startOpacityBase = setArg(
        toNumber(option.StartOpacityBase),
        params.startOpacityBase
      );
      params.startOpacityRandom = setArg(
        toNumber(option.StartOpacityRandom),
        params.startOpacityRandom
      );
      params.reduceOpacity = setArg(
        toNumber(option.ReduceOpacity),
        params.reduceOpacity
      );
      params.endOpacity = setArg(
        toNumber(option.EndOpacity),
        params.endOpacity
      );
    }
  }
  ...
  OriginalWeather.prototype._addSprite = function () {
    ...
    sprite.opacity =
      endOpacity +
      Math.randomInt(startOpacityBase + startOpacityRandom - endOpacity);
  ...
  Spriteset_Map.prototype.updateWeather = function () {
    ...
    if ($gameScreen.isOriginalWeather()) {
      ...
      this._originalWeather.origin.x =
        $gameMap.displayX() * $gameMap.tileWidth();
      this._originalWeather.origin.y =
