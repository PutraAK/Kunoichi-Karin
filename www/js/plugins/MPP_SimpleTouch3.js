...
(function (exports) {
...

  {
    ...
    Params.SelectTypeStatus = JSON.parse(
      parameters["Select Type Status"] || "[]"
    );
    ...
    Params.OutsideTapStatus = JSON.parse(
      parameters["Outside Tap Status _v3"] || "[]"
    );
  ...
  Object.defineProperties(MppOptions, {
    longPress: {
      get: function () {
        return this.getter("mppLongPress");
      },
    },
    selectType: {
      get: function () {
        return this.getter("mppSelectType");
      },
    },
    okType: {
      get: function () {
        return this.getter("mppOkType");
      },
    },
    outsideTap: {
      get: function () {
        return this.getter("mppOutsideTap");
      },
    },
  });
  ...
  MppOptions.getStatus = function (symbol) {
    for (var i = 0; i < this.params.length; i++) {
      if (this.params[i].symbol === symbol) return this.params[i].status;
    }
    return [];
  };
  ...
  Window_Options.prototype.makeCommandList = function () {
    var params = MppOptions.params;
    for (var i = 0; i < params.length; i++) {
      if (params[i].name) this.addCommand(params[i].name, params[i].symbol);
    }
  ...
  Window_Selectable.prototype.contentsHeight = function () {
    if (Alias.WiSe_contentsHeight) {
      ...
      return (
        Window_Base.prototype.contentsHeight.call(this) + this.itemHeight()
      );
    }
  };
  ...
  Window_Selectable.prototype.updateSmoothCursor = function () {
    if (this._cursorMoveDuration > 0) {
      ...
      this._cursorX +=
        ((this._targetCursorX - this._cursorX) * d) / Method.tri(d);
      this._cursorY +=
        ((this._targetCursorY - this._cursorY) * d) / Method.tri(d);
  ...
  //512
  Alias.WiSe_ensureCursorVisible =
